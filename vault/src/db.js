...
const adminIds = (process.env.ADMIN_ROBLOX_IDS || '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
if (adminIds.length) {
  const stmt = db.prepare('UPDATE users SET is_admin = 1 WHERE roblox_id = ?');
  adminIds.forEach((id) => stmt.run(id));
  db.adminRobloxIds = new Set(adminIds);
} else {
  db.adminRobloxIds = new Set();
}
