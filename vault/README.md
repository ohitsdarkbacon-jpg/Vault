...
- Status flows **Searching → Chain found → Waiting for participants → Everyone confirmed → Middleman requested → Completed / Cancelled**. Nothing is agreed until every member individually confirms; any member can cancel; completing closes the underlying posts
- **A middleman is required for the hand-offs**: once everyone confirms, a member requests one and an online, approved middleman (never a trader in the chain) is assigned from the existing middleman network to oversee the swaps in order. Nobody can mark their hand-off done until a middleman is requested (or, only when none are online, explicitly waived)
- **Group chat room**: once the chain is confirmed, all the traders **plus the assigned middleman** share one room to agree on a server and the hand-off order and confirm receipts — the middleman's messages are flagged ⚖️. Read-only after the chain completes. `GET /api/chains/discover`, `POST /api/chains`, `/confirm`, `/request-mm`, `/waive-mm`, `/messages`, `/done`, `/cancel`
...
- **⚖️ Traders center** (`#traders-center`): post a trade you made (text + optional screenshot) and the community votes **W** (win), **F** (fair), or **L** (loss) — live counts, a color meter, and a "Community says" verdict. One vote per user (switch or un-vote any time), you can't rate your own trade, 10 posts/day, owner/admin delete
